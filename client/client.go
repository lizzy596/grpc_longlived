...
type longlivedClient struct {
	client pb.LonglivedClient // client is the long lived gRPC client
...
func (c *longlivedClient) subscribe() (pb.Longlived_SubscribeClient, error) {
	log.Printf("Subscribing client ID: %d", c.id)
	return c.client.Subscribe(context.Background(), &pb.Request{Id: c.id})
}
...
func (c *longlivedClient) unsubscribe() error {
	...
	_, err := c.client.Unsubscribe(context.Background(), &pb.Request{Id: c.id})
	return err
}
...
func (c *longlivedClient) start() {
	...
	var stream pb.Longlived_SubscribeClient
	for {
		if stream == nil {
